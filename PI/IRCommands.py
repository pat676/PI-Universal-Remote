...
The modifications made will alove this to be run as a module instead as __main__
This file contains of constants affecting the reading of IR_Signals. In case
of unstable reading/ playback of signals modifying theese constants migh help.

The code is not thouroghly tested and may contain bugs
"""
...
def read(GPIO, signalName):
    ...
    print("Recording")
    print("Press key for '{}'".format(signalName))
    code = []
    fetching_code = True
    while fetching_code:
        time.sleep(0.1)
    ...
    if CONFIRM:
        ...
        tries = 0
        while not done:
            print("Press key for '{}' to confirm".format(signalName))
            code = []
            fetching_code = True
            while fetching_code:
                time.sleep(0.1)
            ...
            if the_same:
                ...
                if tries <= 3:
                    ...
                    print("Giving up on key '{}'".format(signalName))
                    return False
                time.sleep(0.5)
    else: # No confirm.
        records[arg] = code[:]
